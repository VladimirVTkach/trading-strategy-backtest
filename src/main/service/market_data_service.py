...
class MarketDataService:
    ...
    def get_historical_candlesticks(self,
        ...
        historical_klines = self._binance_client.get_historical_klines(symbol=symbol,
    ...
    def get_current_candlestick(self,
        ...
        if market_type == MarketType.SPOT:
            kline = self._binance_client.get_klines(symbol=symbol,
                                                    interval=kline_interval,
                                                    limit=1)
        else:
            kline = self._binance_client.futures_klines(symbol=symbol,
                                                        interval=kline_interval,
                                                        limit=1)
